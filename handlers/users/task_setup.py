...
async def task_setup(call: CallbackQuery, state: FSMContext):
	...
	await call.message.edit_text("Загрузите фотографию с подписанным УПД:")
...
@dp.message_handler(content_types = [ContentType.PHOTO, ContentType.DOCUMENT], state = TaskSetupState.PutPhoto)
async def task_done_photo(message: types.Message, state: FSMContext):
	...
	if file_id is None:
		...
		if is_done:
			await message.answer("Запись в БД успешно обновлена! Заявка № <b>{}</b>, установлена в статус - <b>УПД подписан</b>".format(str(id_task)),
