...
async def task_edit_distance_done(message: Message, state: FSMContext):
	...
	if distance_done.isdigit():
		...
		is_done = enquiry.update_table(id = id_task, f81971 = int(distance_done))
		if is_done:
...
async def task_act_photo(message: Message, state: FSMContext):
	...
	if file_id is None:
		...
		if is_saved:
			# await message.answer("{}  Файл успешно сохранён и доступен по ссылке - {}".format(emojize(":white_check_mark:"), file_path), disable_web_page_preview = True)
			enquiry = Enquiry()
			is_done = enquiry.update_table(id = id_task, f81771 = STATUS_UPD_SIGNED, f82101 = file_path)
			if is_done:
...
@dp.callback_query_handler(regexp="^task_edit_point#.+")
async def task_edit_upd(call: CallbackQuery, state: FSMContext):
	id_task = call.data.split("#")[1]
	await call.message.edit_text(
		"Заявка № <b>{}</b>\nЗагрузите фотографию точки, нажав на иконку {}, или нажмите кнопку 'Назад', "
		"для отмены:".format(
			id_task,
			emojize(":paperclip:")
			),
	...
	await state.update_data(id_task=id_task)
	await TaskEditState.PutPointPhoto.set()


@dp.message_handler(content_types=ContentType.ANY, state=TaskEditState.PutPointPhoto)
async def task_act_photo(message: Message, state: FSMContext):
	data_state = await state.get_data()
	id_task = data_state.get("id_task")
	content_type = message.content_type
	...
	if file_id is None:
		...
		source_file = await bot.get_file(file_id)
		# print(f"file_link = {file_link}")
		file_path, is_saved = CloudStorage.save_file(source_file["file_path"], create_filename("photo_point", id_task))
		if is_saved:
			# await message.answer("{}  Файл успешно сохранён и доступен по ссылке - {}".format(emojize(":white_check_mark:"), file_path), disable_web_page_preview = True)
			enquiry = Enquiry()
			is_done = enquiry.update_table(id=id_task, f98270=file_path)
			if is_done:
				await message.answer(
					"{} Фото точки по заявке № <b>{}</b>, успешно загружено.".
					format(emojize(":white_check_mark:"), str(id_task)), reply_markup=inline_back_to_setup()
					)
